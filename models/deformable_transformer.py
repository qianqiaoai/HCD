...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, tgt, masks, pos_embeds, query_embed=None):
        ...
        spatial_shapes = []  # 记录特征h和w，方便恢复
        # flatten and update pos embed
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):  # 全部展平
            bs, c, h, w = src.shape
            spatial_shape = (h, w)
            ...
            lvl_pos_embed = pos_embed + self.level_embed[lvl].view(1, 1, -1)  # 位置编码+可学习的编码
        ...
        lvl_pos_embed_flatten = torch.cat(lvl_pos_embed_flatten, 1)  # [bs*t, \sigma(hi*wi), c]
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=src_flatten.device)
        level_start_index = torch.cat(
            (spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))  # 获取每一层特征的起始位置
        ...
        if self.two_stage:
            ...
            query_embed = repeat(query_embed, 'q c -> b t q c', b=b, t=t).flatten(0, 1)  # c=hidden_dim(256)
            reference_points = self.reference_points(query_embed).sigmoid()  # [b t q 2]
            init_reference_out = reference_points
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        with autocast(enabled=False):
            src2, sampling_locations, attention_weights = self.self_attn(self.with_pos_embed(src, pos),
                                                                         reference_points,
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        ...
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios,
                                                     device=src.device)  # torch.Size([5, 5950, 4, 2])
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index,
        ...
        q = k = self.with_pos_embed(tgt, query_pos)  # [bt,q,c]
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False):
        ...
        # self.bbox_embed = [nn.Linear(decoder_layer.d_model, 2) for _ in range(num_layers)]
            ...
            if self.bbox_embed is not None:
                ...
                if reference_points.shape[-1] == 4:
                    ...
                    new_reference_points[..., :2] = tmp[..., :2] + inverse_sigmoid(
                        reference_points)  # Add linear predict motion offsets
                    new_reference_points = new_reference_points.sigmoid()
