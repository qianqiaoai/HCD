...
class HCD(nn.Module):
    """ This is the HCD module that performs referring video object detection """

    def __init__(self, args, backbone, transformer, num_classes, num_queries, num_feature_levels,
                 num_frames, mask_dim,controller_layers, dynamic_mask_channels,pretrained_model_path,
        ...
        self.query_tensors = nn.Parameter(torch.randn(self.num_queries, hidden_dim))  # 可学习参数，query_number,[5,256]
        # Build Text Encoder
        ...
        self.text_proj = FeatureResizer(
            input_feat_size=self.text_encoder.feat_dim,  # 768
        ...
        # Build MSO
        self.mask_refine = MSO(mask_dim=self.dynamic_mask_channels, img_dim=backbone.num_channels[:2],
                               out_dim=self.dynamic_mask_channels)

        self.rel_coord = rel_coord
        # Parameter initialization
        self.init_aux_head()
        self.linear3_16 = nn.Linear(3, 16)
        # # Build CondInst
    ...
    def build_controller(self):
        ...
        for l in range(self.controller_layers):  # controller_layers=2
            if l == 0:
                if self.rel_coord:
                    weight_nums.append((self.in_channels + 2) * self.dynamic_mask_channels)  # dynamic_mask_channels=16
                else:
        ...
        self.controller = MLP(self.hidden_dim, self.hidden_dim, self.num_gen_params,
                              3)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        features, visual_pos= self.backbone(samples, captions)  # get feature through unet
        b = len(captions)
        ...
        text_features, text_sentence_features = self.forward_text(captions, device=visual_pos[0].device)  # RoBERTa
        text_pos = self.text_pos(text_features).permute(2, 0, 1)  # [length, batch_size, c] 位置编码
        ...
        query_tokens = self.query_tensors.unsqueeze(dim=1).repeat_interleave(b, dim=1)  # [q,b,c]
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-multi_scale_level:], visual_pos[-multi_scale_level:])):
            ...
            srcs.append(src_proj_l) # [x4,x8,x16]
            masks.append(mask)
            poses.append(pos_l)
            assert mask is not None

        if self.num_feature_levels > len(srcs): # 一共会有4层多尺度特征进入Deformable Transformer
        ...
        # init_reference: [batch_size*time, num_queries_per_frame, 2] 采样的位置
        # inter_references: [l, batch_size*time, num_queries_per_frame, 4] 采样的位置和偏移
        ...
        memory_fusion = [F.interpolate(x, size=(tar_h, tar_w), mode="bicubic", align_corners=False) for x in
                         memory]  # bt, c, h, w
        memory_fusion_2 = sum(memory_fusion)
        memory_fusion_2 = rearrange(memory_fusion_2, '(b t) c h w->b t c h w', b=b, t=t)
        memory_fusion = torch.cat(memory_fusion, dim=0)
        memory_fusion = rearrange(memory_fusion, '(l b t) c h w->b t c h w l', b=b, t=t)
        mask_features = self.linear3_16(memory_fusion)  # [b,t,c,h,w,d]
        mask_features = rearrange(mask_features, 'b t c h w d-> b t h w d c')
        outputs_seg_masks = []
        outputs_seg_masks_formatcher = []
        for lvl in range(hs.shape[0]):
            query = hs[lvl]  # [b*t,q,c]
            _, q, c = query.shape
            query = rearrange(query, '(b t) q c->b t c q', b=b, t=t)
            query = query.unsqueeze(2).unsqueeze(2)  # [b t 1 1 c q]
            outputs_seg_mask = torch.matmul(mask_features, query)  # [b t h w d q]
            outputs_seg_mask = rearrange(outputs_seg_mask, 'b t h w d q->b (t q) d h w')
            ...
            outputs_seg_mask_2 = self.dynamic_mask_with_coords(memory_fusion_2, dynamic_mask_head_params,
                                                               lvl_references,
                                                               targets)
            outputs_seg_mask = outputs_seg_mask + outputs_seg_mask_2
            outputs_seg_masks.append(outputs_seg_mask)
            # use pixel_shuffle to convert [h1,w1,p^2] to [h1p,w1p]
            outputs_seg_masks_formatcher.append(
                rearrange(F.pixel_shuffle(outputs_seg_mask.flatten(0, 1), 4).squeeze(1), '(b t q) h w -> b t q h w',
                          b=b, t=t, q=self.num_queries))
        ...
        if self.training:
            ...
            if self.aux_loss:
                ...
                pred_masks = [rearrange(x, 'b (t q) c h w -> b q t c h w', t=t)[y] for x, y in
                ...
                outputs_seg_masks_formatcher = [x.transpose(1, 2)[y] for x, y in
