...
class HCD(nn.Module):
    ...
    def build_controller(self):
        ...
        self.controller = MLP(self.hidden_dim, self.hidden_dim, self.num_gen_params,
                              3)  # 3层的前馈神经网络,输入和隐藏层是hidden_dim,num_gen_params是输出层
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        text_features, text_sentence_features = self.forward_text(captions, device=visual_pos[0].device)  # 用另一个文本编码器
        ...
        # init_reference: [batch_size*time, num_queries_per_frame, 2] 采样的位置，num_queries_per_frame表示每帧选取的注意力位置
